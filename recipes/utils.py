...
class RecipeClient(object):
    ...
    def _get(self, path, filters):
        ...
        try:
            ...
            response = self.session.get(
                "https://{}{}?{}".format(self.host, path, filter_str)
            )
    ...
    def search_recipes_complex(
        ...
        filters.update({"number": 5, "limitLicense": "false"})
    ...
    def get_recipe_instructions(self, recipe_id):
        ...
        response = self._get(
            "/recipes/{}/analyzedInstructions".format(recipe_id), filters
        )
        return response
        ...
        if ingredientName == p.name:
            ...
            for u in userpantry:
                if u.pantry_item.api_id == p.api_id:
                    user_has = True
                    message = (
                        "ERROR: Already in Pantry. You can change the quantity instead"
                    )
                    data.update(
                        {
                            "message": message,
            ...
            for i in results2:
                ...
                for j in i["extendedIngredients"]:
                    ...
                    ing = RecipeIngredients(
                        recipe_id=item, name=name, amount=amount, unit=unit, meta=meta
                    )
                    ing.save()
        data = {
...
def prepare_advance_results(results):
    ...
    if len(results["results"]) == 0:
        data = {
            "message": "No results. Try just searching without MyPantry items instead"
        }
        response = JsonResponse(data)
        ...
        try:
            ...
            for j in n["extendedIngredients"]:
                ...
                if j["meta"]:
                    ...
                    meta = ""
                ing = RecipeIngredients(
                    recipe_id=sim, name=name, amount=amount, unit=unit, meta=meta
                )
                ing.save()
        similar.append(sim)
    return similar
